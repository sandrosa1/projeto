...
class Root extends Page{
    ...
    private static function getRootItems($request, &$objPagination){
        ...
        while($objRoot = $results->fetchObject(EntityRoot::class)){
            ...
            $lists .= View::render('admin/modules/roots/list',[
   ...
   public static function getNewRoot($request){
       ...
       $content = View::render('admin/modules/roots/form',[
    ...
    return parent::getPanel('Cadastro - Admin', $content,'roots');
    ...
    */
    public static function setNewRoot($request){
        ...
        if($postVars['password1'] != $postVars['password2']){
            $request->getRouter()->redirect('/admin/roots/new?status=errorP');

        }elseif( strlen($postVars['password2']) < 8){
            $request->getRouter()->redirect('/admin/roots/new?status=errorM');

        }elseif( !preg_match('/^([0-9a-zA-Z]+)$/',$postVars['password2'])){

            $request->getRouter()->redirect('/admin/roots/new?status=errorC');
            ...
            //Redirecio o usúario
            $request->getRouter()->redirect('/admin/roots/'.$objRoot->idRoot.'/edit?status=created');
    ...
    * @param Integer $idRoot
    ...
    public static function getEditRoot($request, $idRoot){
        ...
        if ($_SESSION['admin']['root']['idRoot'] == $idRoot ){

                $objRoot = EntityRoot::getRootById($idRoot);
                //Valida a instancia
                if(!$objRoot instanceof EntityRoot){
                    $request->getRouter()->redirect('/admin/roots');
                }
            ...
            return parent::getPanel('Editar cadastro - Admin', $content,'roots');

        } 
        
    $request->getRouter()->redirect('/admin/roots?status=permissionError');
    ...
    */
    public static function setEditRoot($request, $idRoot){
        ...
        $request->getRouter()->redirect('/admin/roots/'.$objRoot->idRoot.'/edit?status=updated');
    ...
    */
    public static function getDeleteRoot($request, $idRoot){
        ...
        if($_SESSION['admin']['root']['type'] == 3){
            ...
            $content = View::render('admin/modules/roots/delete',[
                'title'        => 'Editar colaborador',
                'rootName'     => $objRoot->rootName, 
                'userName'     => $objRoot->userName, 
                'type'         => $objRoot->type, 
            ...
            return parent::getPanel('Excluir colaborador - Admin', $content,'roots');
    ...
    */
    public static function setDeleteRoot($request, $idRoot){
        ...
        //Redireciona o usúario
        $request->getRouter()->redirect('/admin/roots?status=deleted');
